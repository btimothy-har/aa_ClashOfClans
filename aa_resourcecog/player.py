...
class aPlayer(coc.Player):
    def __init__(self,**kwargs):
        ...
        rushed_troops = sum([(t.minlevel_for_townhall - t.level) for t in self.troops if t.is_rushed and not t.is_super_troop]) + sum([(p.minlevel_for_townhall - p.level) for p in self.hero_pets if p.level < p.minlevel_for_townhall])
    ...
    async def calculate_clangames(self):
        ...
        if self.player.timestamp >= self.games_start and self.player.timestamp < self.games_end:
            ...
            if (new_score - self.last_updated) > 0:
                ...
                if self.score == 0:
                    self.clan_tag = self.player.clan.tag
